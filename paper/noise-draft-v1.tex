...
not necessarily solely due to functional pressure, such as the selection of more
...
iterated learning suggest that the linguistic structure evinced by the outcome of this process
...
from a utterances like ``Jean is tall'', agents may have diverging representations of how tall
...
of $m_1$: $P(m_1 \mid s,t) = \delta_{s \ge t} = (1 - P(m_2 \mid s,t))$. If a speaker observes a
...
systematically noise and learners are not aware of this, some amount of vagueness may be the
natural result.
...
antonym being sent whenever $m$ does not apply. This is a very crude way of modeling use of
...
\paragraph{Results.} Development of an initially monomorphic population consisting only of
...
Speakers are either literal or pragmatic language users. The former select a random true
message, the latter prefer to send the most informative messages from those that are true in
the observed state \citep{grice:1975}. This gives rise to probabilistic speaker behavior
$P(m \mid s,t= \tuple{\text{lexicon},\text{use}})$ which approximates the following choice
...
and applied iterated learning with state-noise for 20 generations. Mean proportion of resulting
pragmatic users of $L_{\text{lack}}$ under different noise signatures are shown in Figure
\ref{fig:quant}. When $\delta$ is small and $\epsilon$ high, iterated noisy transmission can
lead to populations consisting of almost exclusively English-like lexica with pragmatic
language use. Similar results are obtained for larger $k$ or $l$. \mf{CHECK ME: we need
  selection for this result, do we not??? if so, we must mention this, at least in one
  sentence}
